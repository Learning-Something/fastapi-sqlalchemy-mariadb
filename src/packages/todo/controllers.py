...
from packages.todo.schemas import PartialTodoSchema, TodoSchema
...
class TodoControler:
    def __init__(self, todo_service: ServiceInterface[TodoSchema]):
        ...
        self.router.add_api_route('/{id}', self.get_one, methods=['GET'], response_model=TodoSchema)
        self.router.add_api_route('/', self.create, methods=['POST'], response_model=TodoSchema)
        self.router.add_api_route('/{id}', self.update, methods=['PUT'], response_model=TodoSchema)
        self.router.add_api_route(
            '/{id}', self.delete, methods=['DELETE'], response_model=TodoSchema
        )
    ...
    async def get_one(self, todo_id: int, db_session: AsyncSession = Depends(session_db)):
        self.service.repository.db_session = db_session
        return await self.service.get(todo_id)
